...
func addVMTemplatesCommand(c *core.Core) {
	registerVMCommand(c, "vm/templates", core.Command{
		Description: vmT("cmd.vm.templates.short"),
...
func addTemplatesShowCommand(c *core.Core) {
	registerVMCommand(c, "vm/templates/show", core.Command{
		Description: vmT("cmd.vm.templates.show.short"),
		Action: func(opts core.Options) core.Result {
...
func addTemplatesVarsCommand(c *core.Core) {
	registerVMCommand(c, "vm/templates/vars", core.Command{
		Description: vmT("cmd.vm.templates.vars.short"),
		Action: func(opts core.Options) core.Result {
			args := optionArgs(opts)
			if len(args) == 0 {
...
func listTemplates() (
	...
	if len(templates) == 0 {
		core.Println(vmT("cmd.vm.templates.no_templates"))
		return nil
	}

	core.Print(nil, "%s", repoNameStyle.Render(vmT("cmd.vm.templates.title")))
	core.Println()

	w := tabwriter.NewWriter(proc.Stdout, 0, 0, 2, ' ', 0)
	core.Print(w, "%s", vmT("cmd.vm.templates.header"))
	core.Print(w, "%s", "----\t-----------")
	...
	core.Println()
	core.Print(nil, "%s %s", vmT("cmd.vm.templates.hint.show"), dimStyle.Render("core vm templates show <name>"))
	core.Print(nil, "%s %s", vmT("cmd.vm.templates.hint.vars"), dimStyle.Render("core vm templates vars <name>"))
	core.Print(nil, "%s %s", vmT("cmd.vm.templates.hint.run"), dimStyle.Render("core vm run --template <name> --var SSH_KEY=\"...\""))
...
func showTemplateVars(name string) (
	...
	if len(required) > 0 {
		core.Print(nil, "%s", errorStyle.Render(vmT("cmd.vm.templates.vars.required")))
	...
	if len(optional) > 0 {
		core.Print(nil, "%s", successStyle.Render(vmT("cmd.vm.templates.vars.optional")))
	...
	if len(required) == 0 && len(optional) == 0 {
		core.Println(vmT("cmd.vm.templates.vars.none"))
	}
...
func RunFromTemplate(templateName string, vars map[string]string, runOpts container.RunOptions) (
	...
	core.Print(nil, "%s %s", dimStyle.Render(vmT("cmd.vm.label.building")), yamlPath)
...
func buildLinuxKitImage(yamlPath, outputPath string) (
	err error, // result
) {
