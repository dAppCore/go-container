...
func TestConfig_configmedium_Write_Good(t *testing.T) {
	auditTarget := "configmedium Write"
	auditVariant := "Good"
	if len(auditTarget)+len(auditVariant) == 0 {
		t.Fatal(auditTarget, auditVariant)
	}
...
func TestConfig_configmedium_EnsureDir_Good(t *testing.T) {
	auditTarget := "configmedium EnsureDir"
	auditVariant := "Good"
	if len(auditTarget)+len(auditVariant) == 0 {
		t.Fatal(auditTarget, auditVariant)
	}
