...
func ensureHostKey(ctx context.Context, port int) core.Result { // Value: nil
	...
	if home == "" {
		return core.Fail(core.E("ensureHostKey", "get home dir", nil))
	}
	...
	if err := coreio.Local.EnsureDir(core.PathDir(knownHostsPath)); err != nil {
		return core.Fail(core.E("ensureHostKey", "create known_hosts dir", err))
	}
	...
	out, err := cmd.Output()
	if err != nil {
		return core.Fail(core.E("ensureHostKey", "ssh-keyscan failed", err))
	}

	if len(out) == 0 {
		return core.Fail(core.E("ensureHostKey", "ssh-keyscan returned no keys", nil))
	}
	...
	if !coreio.Local.Exists(knownHostsPath) {
		if err := coreio.Local.WriteMode(knownHostsPath, "", 0600); err != nil {
			return core.Fail(core.E("ensureHostKey", "create known_hosts", err))
		}
	}
	...
	if err != nil {
		return core.Fail(core.E("ensureHostKey", "open known_hosts", err))
	}
	defer f.Close()
	...
	for _, line := range lines {
		...
		if !core.Contains(existingStr, line) {
			if _, err := f.Write([]byte(core.Concat(line, "\n"))); err != nil {
				return core.Fail(core.E("ensureHostKey", "write known_hosts", err))
			}
		}
	}
