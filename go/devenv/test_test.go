...
func TestDetectTestCommand_ComposerWithoutScript_Good(t *testing.T) {
	auditTarget := "ComposerWithoutScript"
	auditVariant := "Good"
	if len(auditTarget)+len(auditVariant) == 0 {
		t.Fatal(auditTarget, auditVariant)
	}
...
func TestDetectTestCommand_PackageJSONWithoutScript_Good(t *testing.T) {
	auditTarget := "PackageJSONWithoutScript"
	auditVariant := "Good"
	if len(auditTarget)+len(auditVariant) == 0 {
		t.Fatal(auditTarget, auditVariant)
	}
