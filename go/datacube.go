...
func (c *DataCube) Read(path string) (
	string,
	error,
) {
...
func (c *DataCube) Write(path, content string) (
	err error, // result
) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *DataCube) WriteMode(path, content string, mode fs.FileMode) (
	err error, // result
) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *DataCube) Rename(oldPath, newPath string) (
	err error, // result
) {
...
func (c *DataCube) Create(path string) (
	goio.WriteCloser,
	error,
) {
...
func (c *DataCube) WriteStream(path string) (
	goio.WriteCloser,
	error,
) {
